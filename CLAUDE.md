...
**Auditing is automatic.** A Prisma Client extension (`lib/audit/audit-extension.ts`, wired in `lib/prisma.ts`) records every `create`/`update`/`delete` on almost every model into `AuditLog` — capturing the acting user, the before/after snapshots (`datosPrevios`/`datosNuevos`), and the changed fields (`detalle.cambios`). The "who" comes from the per-request context (`lib/api/request-context.ts`, an `AsyncLocalStorage` that `withAuth` populates with `userId`). Because it's automatic, **do NOT call `logAudit()` manually** in routes (it would double-log); the helper in `lib/api/audit-log.ts` stays only for rare explicit/semantic entries. Excluded models: `AuditLog`, `Session`, `Account`, `VerificationToken`. Query a record's history via `GET /api/audit/historial?tabla=<Model>&rowPk=<id>` (scoped by org; platform admin sees all). Note: audit writes are best-effort and happen outside the caller's transaction, so a rolled-back tx may still leave an audit row.
