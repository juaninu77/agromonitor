    ...
    return (
      <Card className="border border-border">
        <CardContent className="p-12 text-center">
          ...
          <h3 className="text-lg font-semibold text-foreground mb-2">No hay animal seleccionado</h3>
          <p className="text-muted-foreground mb-4">Selecciona un animal de la lista para ver sus detalles</p>
          {totalAnimals === 0 && (
      ...
      <CardContent className="p-6">
        <Tabs defaultValue="peso" className="w-full">
          <TabsList className="grid w-full grid-cols-4 border border-border">
            <TabsTrigger value="peso" className="border-r-2 border-border">Peso</TabsTrigger>
            <TabsTrigger value="reproduccion" className="border-r-2 border-border">Reproducción</TabsTrigger>
            <TabsTrigger value="salud" className="border-r-2 border-border">Salud</TabsTrigger>
                ...
                <CardContent className="pt-4">
                  <div className="grid grid-cols-2 gap-4">
                    <div className="text-center p-4 border-2 border-gray-100 rounded-lg">
                      <p className="text-3xl font-bold text-primary">{selectedAnimal.weight || "—"}</p>
                      <p className="text-sm text-muted-foreground">kg Peso Vivo</p>
                    </div>
                    <div className="text-center p-4 border-2 border-gray-100 rounded-lg">
                      <p className="text-3xl font-bold text-green-600">
                        {selectedAnimal.dailyGain ? `+${selectedAnimal.dailyGain}` : "—"}
                      </p>
                      <p className="text-sm text-muted-foreground">kg/día GDP</p>
                    </div>
                    <div className="text-center p-4 border-2 border-gray-100 rounded-lg">
                      <p className="text-3xl font-bold text-purple-600">
                        {selectedAnimal.bodyConditionScore || "—"}
                      </p>
                      <p className="text-sm text-muted-foreground">CC (1-9)</p>
                    </div>
                    <div className="text-center p-4 border-2 border-gray-100 rounded-lg">
                      ...
                      selectedAnimal.weightRecords.map((record: any, index: number) => (
                        <div key={index} className="flex items-center justify-between p-3 border-2 border-gray-100 rounded-lg">
                          <div>
                            ...
                            <p className="text-sm text-muted-foreground">GDP: +{record.gain} kg/día | CC: {record.bodyCondition}</p>
                          </div>
                          <div className="text-right">
                            <p className="text-lg font-bold text-primary">{record.weight} kg</p>
                ...
                <CardContent className="pt-4">
                  ...
                  <div className="space-y-4">
                    <div className="flex justify-between items-center p-3 border-2 border-gray-100 rounded-lg">
                      <span className="text-muted-foreground">Descendencia total:</span>
                      <span className="font-semibold">{selectedAnimal.offspring || 0}</span>
                    </div>
                    <div className="flex justify-between items-center p-3 border-2 border-gray-100 rounded-lg">
                      <span className="text-muted-foreground">Progenie esperada:</span>
                      ...
                      selectedAnimal.healthRecords.map((record: any, index: number) => (
                        <div key={index} className="p-3 border-2 border-gray-100 rounded-lg">
                          ...
                          <div className="flex justify-between items-center text-sm">
                            <span className="text-muted-foreground">Veterinario: {record.veterinarian}</span>
          ...
          <TabsContent value="genetica" className="mt-6">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <Card className="border border-border">
                ...
                <CardContent className="pt-4">
                  <div className="text-center mb-6">
                    ...
                    <p className="text-muted-foreground">Índice Genético Total</p>
                  </div>
                  <div className="space-y-3">
                      ...
                      Object.entries(selectedAnimal.geneticData.breedingValues).map(([trait, value], index) => {
                        ...
                        return (
                          <div key={index} className="flex justify-between items-center p-3 border-2 border-gray-100 rounded-lg">
                            ...
                            <span
                              className={`font-semibold ${valueStr.startsWith("+") ? "text-green-600" : valueStr.startsWith("-") ? "text-red-600" : "text-primary"}`}
                            >
                              {valueStr}
                            </span>
                          </div>
              ...
              <Card className="border border-border">
                ...
                <CardContent className="pt-4">
                  <div className="space-y-4">
                    <div className="text-center p-4 border-2 border-blue-100 bg-primary/10 rounded-lg">
                      <h4 className="font-semibold text-blue-800">Animal</h4>
                      <p className="text-lg font-bold">{selectedAnimal.name}</p>
                      <p className="text-sm text-muted-foreground">#{selectedAnimal.tagNumber}</p>
                    </div>
                    <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="text-center p-3 border-2 border-gray-100 rounded-lg">
                      <h4 className="font-semibold text-foreground">Abuelo Materno</h4>
