...
type Crop={id:string;forraje:{nombre:string};sector:{nombre:string};desde:string;hasta:string|null;superficieHa:number|null;estado:string;version:number;notas:string|null}
type Reserve={id:string;nombre:string;forraje:{nombre:string};stock:string;minimo:string;unidad:string;ubicacion:string;movimientos:{id:string;fecha:string;tipo:string;cantidad:string;motivo:string}[]}
...
function Workspace({campo,nombre,initialMode}:{campo:string;nombre:string;initialMode:"flota"|"cultivos"}){
  const [mode,setMode]=useState<Mode>(initialMode),[rows,setRows]=useState<Row[]>([]),[page,setPage]=useState(1),[total,setTotal]=useState(0),[q,setQ]=useState("")
  const [loading,setLoading]=useState(true),[error,setError]=useState(""),[notice,setNotice]=useState(""),[revision,setRevision]=useState(0)
  const [catalog,setCatalog]=useState<{forrajes:{id:string;nombre:string}[];sectores:{id:string;nombre:string}[];puedeEditar:boolean}>({forrajes:[],sectores:[],puedeEditar:false})
  ...
  useEffect(()=>{
    ...
    Promise.all([
      fetch(`/api/campo/${mode}?establecimientoId=${campo}&page=${page}&q=${encodeURIComponent(q)}`,{signal:c.signal}).then(async r=>{const b=await r.json();if(!r.ok)throw Error(b.error);return b}),
    ...
    return()=>c.abort()
  },[campo,mode,page,q,revision])
  function begin(next:Action,row:Row|null=null,tipo="entrada"){
    setAction(next);setSelected(row);setFormError("");setNotice("")
    setDraft({fecha:today(),desde:today(),hasta:today(),tipo:next==="flota"?"vehiculo":tipo,unidad:next==="flota"?"km":"fardos",lectura:row&&"lectura"in row?row.lectura:"0",minimo:"0",moneda:"ARS",clave:crypto.randomUUID()})
  }
  ...
  async function save(e:FormEvent){e.preventDefault();if(!action)return;setSaving(true);setFormError("")
    try{
      ...
      for(const f of fields()){const value=draft[f.key]??"";if(f.key==="densidadSiembraKgHa"&&!value)continue;data[f.key]=value}
  ...
  function switchMode(m:Mode){setMode(m);setPage(1);setQ("");setRows([]);setAction(null);setNotice("")}
