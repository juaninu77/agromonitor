...
import { Search, Loader2, LocateFixed, Plus, MoreHorizontal, Maximize, Minimize, PanelRightClose, PanelRightOpen, Undo2, Redo2 } from "lucide-react"
...
export default function MapWorkspace({ fieldId, onList }: { fieldId: string; onList: () => void }) {
  ...
  const [panelEditing, setPanelEditing] = useState(false), [expanded, setExpanded] = useState(false), [panel, setPanel] = useState(true)
  ...
  return <div className={expanded ? "fixed inset-0 z-40 flex h-dvh flex-col gap-2 bg-background p-3" : "relative flex h-[calc(100dvh-244px)] min-h-[560px] flex-col gap-2 md:h-[calc(100dvh-212px)] md:min-h-[360px]"}>
    {expanded && <p className="text-sm font-semibold">Mapa · {establecimientoActivo?.nombre}</p>}
    <div className="flex shrink-0 flex-wrap items-center gap-2">
      {canEdit && <DropdownMenu><DropdownMenuTrigger asChild><Button disabled={!!draft || panelEditing}><Plus className="mr-2 h-4 w-4"/>Agregar</Button></DropdownMenuTrigger><DropdownMenuContent align="start">
      ...
      <Button variant="outline" onClick={() => setShowLocation(!showLocation)} disabled={!!draft || panelEditing}><Search className="mr-2 h-4 w-4"/>Ubicar campo</Button>
      <Button variant="outline" size="icon" aria-label="Ver todos los lugares del campo" title="Ver todo el campo" onClick={() => { setFilter("todos"); setStateFilter("todos"); setPlaceQuery(""); setSelected(null); setFitKey(v => v + 1) }} disabled={!!draft || panelEditing || !sectors.some(s => s.geometria)}><LocateFixed className="h-4 w-4"/></Button>
      ...
      <Button variant="outline" size="icon" aria-label={panel ? "Ocultar panel de lugares" : "Mostrar panel de lugares"} disabled={!!draft || panelEditing} onClick={() => setPanel(v => !v)}>{panel ? <PanelRightClose className="h-4 w-4"/> : <PanelRightOpen className="h-4 w-4"/>}</Button>
      <DropdownMenu><DropdownMenuTrigger asChild><Button variant="ghost" size="icon" aria-label="Más opciones del mapa"><MoreHorizontal className="h-4 w-4"/></Button></DropdownMenuTrigger><DropdownMenuContent>
        ...
        <DropdownMenuItem onSelect={onList} disabled={!!draft || panelEditing}>Ver listado de lugares</DropdownMenuItem>
      </DropdownMenuContent></DropdownMenu>
    ...
    <div className="flex shrink-0 flex-wrap items-center gap-x-4 gap-y-1 text-xs text-muted-foreground"><span><strong className="text-foreground">{sectors.filter(s => isParcel(s.tipo) && s.bovinos + s.ovinos > 0).length}</strong> parcelas con ganado</span><span>{sectors.reduce((n, s) => n + s.bovinos, 0)} bovinos · {sectors.reduce((n, s) => n + s.ovinos, 0)} ovinos</span><span>{sectors.reduce((n, s) => n + s.pendientes, 0)} {sectors.reduce((n, s) => n + s.pendientes, 0) === 1 ? "tarea pendiente" : "tareas pendientes"}</span></div>
    ...
    {showLocation && <div className="absolute left-2 right-2 top-14 z-[600] max-h-[60dvh] space-y-3 overflow-y-auto rounded-xl border bg-card p-4 shadow-lg md:right-auto md:w-[440px]">
    ...
    <div className={`grid min-h-0 flex-1 gap-3 ${panel || draft ? "grid-rows-[minmax(160px,0.8fr)_minmax(260px,1fr)] md:grid-cols-[minmax(0,1fr)_350px] md:grid-rows-1 xl:grid-cols-[minmax(0,1fr)_380px]" : "grid-rows-1"}`}>
      <div className="flex min-h-0 min-w-0 flex-col gap-2">
        ...
        <div className="min-h-0 flex-1"><FieldMap sectors={visible} selected={selected} draft={saving ? null : draft} focus={focus} satellite={satellite} fitKey={fitKey} onSelect={id => { if (!panelEditing) { setSelected(id); setPanel(true) } }} onVertices={changeVertices} colorByState={colorByState} /></div>
        <p className="shrink-0 text-[11px] leading-tight text-muted-foreground">{sectors.filter(s => s.geometria).length} de {sectors.length} lugares ubicados · Imágenes de referencia, no en tiempo real · Superficie aproximada, no catastral.</p>
      </div>
      {(panel || draft) && <aside className="min-h-0 overflow-hidden rounded-xl border bg-card" aria-label={current && !draft ? "Ficha del lugar seleccionado" : "Lugares y filtros"}>
        {draft ? <form onSubmit={e => { e.preventDefault(); void save() }} className="h-full space-y-3 overflow-y-auto p-4">
          ...
          <label className="flex items-center gap-2 text-xs text-muted-foreground"><input type="checkbox" checked={colorByState} onChange={e => setColorByState(e.target.checked)}/>Colorear por estado</label>{colorByState && <p className="text-xs text-muted-foreground">Azul: ganado · Violeta: descanso · Ocre: cultivo · Gris: otros</p>}
